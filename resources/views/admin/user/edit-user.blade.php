	...
	<!--begin::Card-->
			<div class="card card-custom ">
				...
				<div class="row">
					...
					<form action="{{ route('update-users',$getUser->id) }}" method="POST" name="status_form{{ $getUser->id }}">
					@csrf
					@method('POST')
					...
					<?php
						...
						?>
						<input type="text" name="id" value="{{$getUser->id}}" hidden="">
					<div class="card-body">
						<div class="form-group">
							<label>Full Name</label>
							<input id="name" type="email" class="form-control  @error('name') is-invalid @enderror" name="name" value="{{ $getUser->name }}" placeholder="Full Name" required autocomplete="name"/>
						...
						<div class="form-group">
							<label>Email</label>
							<input id="email" type="email" class="form-control  @error('email') is-invalid @enderror" name="email" value="{{ $getUser->email }}" placeholder="Email" required autocomplete="email"/>
						...
						<div class="row">
							...
							@foreach($get_group as $get)
									<option value="{!!$get->access_group_id!!}" @if($getUser->access_group_id == $get->access_group_id) selected="" @endif>{{ $get->access_group_name }}</option>
								@endforeach
					...
					<div class="card-footer">
						<button type="button" class="btn btn-primary mr-2 is_active" data-form="{{ $getUser->id  }}" >Submit</button>
